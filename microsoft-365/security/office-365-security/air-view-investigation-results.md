...
|Threats found | The threats found status means that the investigation found issues, but it has no actions available to resolve them.  Typically this means some user activity has been found, where the user has done something, but no cleanup is available – e.g. DLP event, sending anomaly, sent malware or phish.  The investigation found no malicious URL, file or emails to remediate, as well as no mailbox activity it could cleanup (e.g. forwarding rules or delegation).<br/><br/> ‘Threats found’ happens most often with legitimate emails reported by users or manually triggered investigations.  It also happens if the ATP analysis does not find malicious content in the emails, most often when the threat has been deactivated or removed from the internet location is was hosted on.  If you believe a URL or file was not properly detected as malicious, create an admin submission for the email or for the specific payload.  Then take manual action on the emails using Explorer to find and remediate them.​ |
